...
const ScrollScaleGameCard = ({ game, navigate }) => {
  return (
    <div
      className="
        relative w-full 
        h-[300px] sm:h-[400px] md:h-[450px] lg:h-[550px] 
    ...
    >
      {/* ===== Game Image ===== */}
      <img
        src={gameImages[game.title] || "/default-room.jpg"}
        alt={game.title}
      ...
      >
        ...
        <p className="text-sm sm:text-base opacity-90">
          ₹{game.price?.toLocaleString()} / hour
        </p>
      </div>

      {/* ===== Gradient overlay for readability (mobile only) ===== */}
      <div className="absolute bottom-0 left-0 w-full h-[40%] bg-gradient-to-t from-black/60 to-transparent sm:hidden" />
    </div>
  );
};

export default ScrollScaleGameCard;
